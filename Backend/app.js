...
app.post('/login', async (req, res) => {
  try {
    ...
    if (!user) {
      return res.json({ error: "User does not exist, create an account" });
    }
...
// --- Recipe routes (add, edit, delete, fetch, etc.) ---
app.post("/addrecipe", upload.single("photo"), async (req, res) => {
  try {
    const { title, description, username, breakfast, lunch, dinner } = req.body;
    const steps = req.body.steps ? [].concat(req.body.steps) : [];
...
// ✅ Include your other recipe routes here (edit, delete, getRecipes, etc.)
